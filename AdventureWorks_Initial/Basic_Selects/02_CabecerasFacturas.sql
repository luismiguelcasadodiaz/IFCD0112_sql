...
-- 07 Pedidos con el número de cuenta que empieza por "10-4020-00" seguido de un caracter, seguido de un "7" y cualquier número de carácteres.
USE [AdventureWorks2022];
SELECT SalesOrderID, AccountNumber  FROM [Sales].[SalesOrderHeader]
WHERE AccountNumber LIKE '10-4020-00_7%'

-- 08 Pedidos en los que coincide la dirección de envñio con la dirección de facturación
...
SELECT * FROM [AdventureWorks2022].[Sales].[SalesOrderHeader]
WHERE PurchaseOrderNumber LIKE 'PO1%' AND CreditCardApprovalCode  LIKE '%23Vi3%';
...
-- 24 Para los Pedidos con un subtotal superior a 10000 y un coste de flete superior al 5% del subtotal, muestra  en cuantos dólares el flete excede al 5% del subtotal
SELECT SalesOrderID, PurchaseOrderNumber, SubTotal, SubTotal * 0.05 as fivePercent, Freight,  Freight - (SubTotal * 0.05) as excess FROM [AdventureWorks2022].[Sales].[SalesOrderHeader]
